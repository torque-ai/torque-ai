...
test_remote_bootstrap_streams_runner_output_without_inherited_stdout_hang() {
  ...
  expect_contains "remote tail streams from output tempfile" "$RUN_BOOTSTRAP_SH" "tail -n +1 -f --pid=\"\$pid\" \"\$out\""
  expect_contains "tail output is line-counted for replay" "$RUN_BOOTSTRAP_SH" "awk -v state=\"\$streamed_lines_file\""
  expect_contains "missed tail output is replayed before cleanup" "$RUN_BOOTSTRAP_SH" "tail -n +\"\$((streamed_lines + 1))\" \"\$out\""
