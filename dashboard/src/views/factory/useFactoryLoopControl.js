...
export function useFactoryLoopControl() {
  ...
  const [automationReadiness, setAutomationReadiness] = useState(null);
  ...
  const loadProjects = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      const response = await factoryApi.projects({
        includeIdleDiagnosis: true,
        includeAutomationReadiness: true,
      });
      const nextProjects = getProjectsFromResponse(response);
      setProjects(nextProjects);
      setIdleDiagnosis(response?.idle_diagnosis || null);
      setAutomationReadiness(response?.automation_readiness || null);
