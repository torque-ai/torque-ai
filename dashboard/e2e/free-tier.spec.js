...
test.beforeEach(async ({ page }) => {
  await page.route('**/api/free-tier/status', (route) => {
    route.fulfill({
      status: 200,
  ...
  await page.route('**/api/free-tier/history*', (route) => {
    route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  await page.route('**/api/**', (route) => {
    ...
    // Let free-tier routes through (handled above)
    if (url.includes('/api/free-tier/')) {
      route.fallback();
      return;
    }
    route.fulfill({
