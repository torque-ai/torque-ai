/**
 * Runtime database store for TORQUE task persistence.
 *
 * STATUS: This module owns the runtime database connection and the remaining
 * facade-shaped compatibility surface. All 47 sub-modules now have factory
 * exports (createXxx) and are registered in the DI container
 * (server/container.js). New feature code should use the container:
 *
 *   const { defaultContainer } = require('./container');
 *   const hostMgmt = defaultContainer.get('hostManagement');
 *
 * The legacy `database.js` entrypoint is now a shim to this module. This
 * module still merges all sub-module exports into a flat namespace for
 * backward compatibility with consumers that need the runtime facade while
 * they migrate to the container.
 *
 * Migration guide:
