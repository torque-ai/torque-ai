...
function buildCodexCommand(task, resolvedFileContext, providerConfig, opts = {}) {
    ...
    // plan_quality_review/replan_rewrite/verify_review run on `low`,
    // generic factory_internal + scouts run on `high`, shell-execute-only
    // tasks run on `low`, everything else falls through to the user's
    // xhigh default. Mirrors the same logic in command-builders.js.
