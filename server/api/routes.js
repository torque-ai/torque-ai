...
const routes = [
  ...
  // Command Center batch endpoints — MUST precede the `/api/v2/tasks/:task_id` regex
  // below, otherwise the summary slugs get interpreted as a task_id and the
  // getTask handler returns 404.
  {
    method: 'GET',
    path: '/api/v2/tasks/command-center-summary',
    handlerName: 'handleV2CpCommandCenterSummary',
    middleware: buildV2Middleware(),
  },
  {
    method: 'GET',
  ...
  // ─── Supplement: auto-generated routes for tools added since the last
  // hand-tuning pass on routes-passthrough.js. The generator only emits
  // routes for tools that are NOT already covered above, so this list is
  // strictly additive. Re-run scripts/generate-rest-routes.js after
  // adding a new tool-defs entry to keep it current.
  ...require('./routes-generated-supplement'),
];
