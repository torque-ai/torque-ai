...
function extractSymbols(source, filePath) {
  return extractJs(source, filePath);
}

function extractJs(source, filePath) {
...
function parseJavaScript(content, filePath) {
  return {
    ...
    imports: extractJavaScriptImports(content, filePath),
    exports: extractJavaScriptExports(content),
  };
}
...
function extractJavaScriptImports(content, filePath) {
  const dependencies = [];
  const patterns = [
