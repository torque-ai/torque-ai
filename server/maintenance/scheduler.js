  ...
  try {
    switch (taskType) {
      ...
      case 'cleanup_stale_tasks': {
        ...
        const result = db.cleanupStaleTasks(staleRunningMin, staleQueuedMin, 5, restartResubmissionMin);
        ...
        if (result.total > 0 || dormant.workflows_cancelled > 0) {
          debugLog(`Stale task cleanup: ${result.running_cleaned} running, ${result.queued_cleaned} queued, ${result.workflow_task_cleaned || 0} workflow child, ${result.restart_resubmission_cleaned || 0} restart resubmission, ${dormant.workflows_cancelled || 0} dormant workflow`);
        }
        break;
      }
