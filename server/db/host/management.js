...
function setDb(dbInstance) {
  ...
  hostCapacity.setDb(dbInstance);
  hostCapacity.setHostFns({
    getOllamaHost,
    listOllamaHosts,
    updateOllamaHost,
...
// ── Capacity Gating Delegates (canonical implementation in host-capacity.js) ──
 ...
 * @param {{ db: object, taskCore?: object, projectConfigCore?: object }} deps
 */
function createHostManagement({ db: dbInstance, taskCore, projectConfigCore }) {
  setDb(dbInstance);
  setGetTask(taskCore?.getTask || (() => null));
  setGetProjectRoot(projectConfigCore?.getProjectRoot || (() => null));
  return {
    addOllamaHost,
    getOllamaHost,
    getOllamaHostByUrl,
