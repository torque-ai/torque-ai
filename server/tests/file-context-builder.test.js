...
describe('execution/file-context-builder', () => {
  ...
  describe('ensureTargetFilesExist', () => {
    it('creates stub .js files', () => {
      const mod = loadInitializedSubject();
      const createdPaths = mod.ensureTargetFilesExist(workingDir, ['nested/new-file.js']);
      const absPath = path.resolve(workingDir, 'nested/new-file.js');

      expect(createdPaths).toEqual([absPath]);
      expect(fs.existsSync(absPath)).toBe(true);
      expect(fs.readFileSync(absPath, 'utf8')).toBe('// Placeholder — to be generated by LLM\n');
    });

    it('skips paths outside the working directory', () => {
      const mod = loadInitializedSubject();
      ...
      const createdPaths = mod.ensureTargetFilesExist(workingDir, [relPath]);
    ...
    it('does not overwrite existing files', () => {
      ...
      const createdPaths = mod.ensureTargetFilesExist(workingDir, ['src/existing.js']);

      expect(createdPaths).toEqual([existingPath]);
      expect(fs.readFileSync(existingPath, 'utf8')).toBe('console.log("keep me");\n');
    });
  });
});
