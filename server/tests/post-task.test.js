...
`.trim());
    ...
    it('allows placeholder-looking fixture text inside test files', () => {
      const workingDir = makeWorkDir('quality-test-fixture-placeholder');
      const filePath = writeFile(workingDir, 'server/tests/file-context-builder.test.js', `
import { describe, expect, it } from 'vitest';

describe('fixtures', () => {
  it('keeps placeholder-looking source samples as data', () => {
    const source = \`
// TODO: implement parser
function run() {
  return true;
...
`.trim());
  ...
  describe('checkSyntax', () => {
    ...
    it('retries import-based JavaScript syntax checks as ESM', () => {
      const workingDir = makeWorkDir('syntax-esm-test');
      fs.writeFileSync(path.join(workingDir, 'package.json'), '{"type":"commonjs"}\n', 'utf8');
      const esmTestPath = writeFile(workingDir, 'server/tests/file-context-builder.test.js', `
import { describe, expect, it } from 'vitest';
...
`.trim());
      ...
      mockExecFileSync.mockImplementation((cmd, args, options) => {
        if (cmd === 'node' && args[0] === '--check' && args[1] === esmTestPath) {
          const err = new Error('Command failed: node --check file-context-builder.test.js');
          err.stderr = Buffer.from('SyntaxError: Cannot use import statement outside a module');
          throw err;
        }
        if (cmd === 'node' && args[0] === '--input-type=module' && args[1] === '--check') {
          expect(options.input).toContain("import { describe, expect, it } from 'vitest';");
          return '';
        }
        throw new Error(`Unexpected call: ${cmd} ${args.join(' ')}`);
      });

      const result = mod.checkSyntax(workingDir, ['server/tests/file-context-builder.test.js']);
