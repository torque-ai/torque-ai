...
`);
  ...
  it('hydrates dependencies when reusing an active plan artifact worktree', async () => {
    ...
    const worktreePath = path.join(project.path, '.worktrees', 'feat-reuse-deps');
    const packageJson = JSON.stringify({ dependencies: { leftpad: '*' } });
    ...
    factoryWorktrees.recordWorktree({
      ...
      vc_worktree_id: 'vc-reuse-deps',
      branch: `feat/factory-${workItem.id}-reuse-deps`,
      worktree_path: worktreePath,
    });

    const result = await loopController.__testing__.prepareAutoGeneratedPlanArtifactWorktree({
      project,
      workItem,
    });

    expect(result.workingDirectory).toBe(worktreePath);
    expect(fs.existsSync(path.join(worktreePath, 'node_modules', 'leftpad', 'package.json'))).toBe(true);
    expect(listDecisionRows(db, project.id).map((row) => row.action)).toContain('plan_generation_worktree_reused');
  });
