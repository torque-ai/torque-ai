...
`, 'utf8');
  ...
  it('reuses a materialized generated plan instead of re-awaiting a stale failed plan-generation task', async () => {
    const { project, workItem } = registerExecuteProject({
      description: 'Add coverage for stale failed plan-generation metadata after a fresh architect plan exists.',
      origin: {
        plan_generation_task_id: 'old-failed-plan-gen-task',
        plan_generation_status: 'failed',
        plan_generator_provider: 'ollama',
      },
    });
    const darkProject = { ...project, trust_level: 'dark' };
    const batchId = `factory-${project.id}-stale-failed-plan-task`;
    const instance = factoryLoopInstances.createInstance({
      project_id: project.id,
      work_item_id: workItem.id,
      batch_id: batchId,
    ...
    initializeCleanGitWorktree(prepared.workingDirectory);
    fs.mkdirSync(path.join(prepared.workingDirectory, 'server', 'factory'), { recursive: true });
    fs.writeFileSync(path.join(prepared.workingDirectory, 'server', 'factory', 'plan-execute.js'), 'module.exports = {};\n');
    const proposalOutput = JSON.stringify({
      file_edits: [{
        file: 'server/factory/plan-execute.js',
        operations: [{
          type: 'replace',
          old_text: 'const staleTerminalGeneration = false;',
          new_text: 'const staleTerminalGeneration = true;',
        }],
      }],
    }, null, 2);
    const materializedProject = { ...darkProject, path: prepared.workingDirectory };
    const materializedPlan = loopController._internalForTests.normalizeAutoGeneratedPlanMarkdown(
      `\`\`\`json\n${proposalOutput}\n\`\`\``,
      workItem,
      materializedProject,
    );
    ...
    taskCore.getTask = vi.fn((taskId) => {
      if (taskId === 'old-failed-plan-gen-task') {
        return {
          id: taskId,
          status: 'failed',
          provider: 'ollama',
          error_output: 'Ollama chat request timed out',
          tags: planGenerationTags(project.id, workItem.id),
          metadata: planGenerationMetadata(project.id, workItem.id),
        };
      }
    ...
    try {
      const result = await loopController.executeNonPlanFileStage(
        darkProject,
        factoryLoopInstances.getInstance(instance.id),
        factoryIntake.getWorkItem(workItem.id)
      );
      const updatedWorkItem = factoryIntake.getWorkItem(workItem.id);

      expect(result).toMatchObject({
        reason: 'reused auto-generated plan',
        work_item: expect.objectContaining({ id: workItem.id }),
      });
      expect(gateSpy).toHaveBeenCalledWith(expect.objectContaining({
        plan: materializedPlan,
        workItem: expect.objectContaining({ id: workItem.id }),
      ...
      expect(updatedWorkItem.origin?.plan_generation_task_id).toBeUndefined();
      expect(updatedWorkItem.origin?.plan_generation_status).toBeUndefined();
      expect(updatedWorkItem.origin?.plan_generator_provider).toBeUndefined();

      const decisions = listDecisionRows(db, project.id);
      expect(decisions.find((row) => row.action === 'stale_plan_generation_task_ignored_for_materialized_plan'))
        .toMatchObject({
          stage: 'execute',
          outcome: expect.objectContaining({
            work_item_id: workItem.id,
            generation_task_id: 'old-failed-plan-gen-task',
            task_status: 'failed',
            plan_path: prepared.planPath,
          }),
        });
  ...
  it('does not route heavyweight validation through torque-remote when remote tests are disabled', async () => {
    const { project, workItem, projectDir } = registerExecuteProject({
      description: 'Reformat LAN startup retry test bodies without changing behavior.',
    });
    const targetFile = path.join(projectDir, 'simtests/Netcode/LanStartupCoordinatorTests.cs');
    ...
    const localProject = {
      ...
      config: {
        ...
        verify_command: 'dotnet test simtests/SimCore.DotNet.Tests.csproj -c Release --filter "Category!=RequiresAndroidBuild"',
      },
    };
    ...
    expect(normalized).toContain('Acceptance criteria: `git diff --check` is clean and `git diff --check` passes.');
    expect(normalized).not.toContain('dotnet test simtests/SimCore.DotNet.Tests.csproj');
    expect(normalized).not.toContain('torque-remote dotnet test');

    const deterministic = planQualityGate.runDeterministicRules(normalized, {
      repoPath: project.path,
      projectConfig: localProject.config,
    });
    expect(deterministic.hardFails).toEqual([]);
  });

  it('rejects torque-remote validation when project remote tests are disabled', async () => {
    const { project, workItem } = registerExecuteProject({
      description: 'Reformat LAN startup retry test bodies without changing behavior.',
    });
    const plan = [
      ...
      '- [ ] **Step 1: Patch and validate the test formatting**',
      '',
      '    Edit `simtests/Netcode/LanStartupCoordinatorTests.cs` only. Estimated scope is one file and about 20 lines. Acceptance criteria: `torque-remote dotnet test simtests/SimCore.DotNet.Tests.csproj -c Release --filter LanStartupCoordinatorTests` passes.',
    ].join('\n');

    const deterministic = planQualityGate.runDeterministicRules(plan, {
      repoPath: project.path,
      projectConfig: {
        prefer_remote_tests: false,
        verify_command: 'dotnet test simtests/SimCore.DotNet.Tests.csproj -c Release --filter "Category!=RequiresAndroidBuild"',
    ...
    expect(deterministic.hardFails).toEqual(expect.arrayContaining([
      expect.objectContaining({ rule: 'task_avoids_disabled_remote_validation' }),
    ]));
  });
  ...
  it('preserves explicit create-file test targets instead of replacing them with unrelated basename matches', async () => {
    const { project, workItem, projectDir } = registerExecuteProject({
      title: '[LOW] Small route-exposed shims remain uncovered and may drift from their canonical implementations',
      description: [
        'File: `server/handlers/budget-handlers.js`; `server/handlers/provider-scoring-handlers.js`; `server/handlers/discovery-handlers.js`.',
        'Description: These files are small, but they are live tool surfaces via `server/tools.js`.',
      ].join(' '),
    });
    for (const repoFile of [
      'agent/tests/run.test.js',
      'server/handlers/budget-handlers.js',
      'server/handlers/provider-scoring-handlers.js',
      'server/handlers/discovery-handlers.js',
    ]) {
      const absolute = path.join(projectDir, repoFile);
      fs.mkdirSync(path.dirname(absolute), { recursive: true });
      fs.writeFileSync(absolute, '// fixture\n');
    ...
    const rawPlan = [
      '# Shim coverage plan',
      '**Source:** auto-generated from work_item #2297',
      '**Tech Stack:** Node.js',
      '',
      '## Task 1: Add direct tests for budget and provider-scoring handler shims',
      '',
      '- [ ] **Step 1: Read shim modules**',
      '',
      '    Read `server/handlers/budget-handlers.js` and `server/handlers/provider-scoring-handlers.js` to identify container delegation behavior. Estimated scope: two source files and one new test file. Acceptance criteria: each exported shim has a happy-path and container-error assertion.',
      '',
      '- [ ] **Step 2: Create test file for budget and provider-scoring handler shims**',
      '',
      '    Create `server/tests/budget-handlers.test.js` with ~6 tests for the two handler modules. Validation: run `npx vitest run server/tests/budget-handlers.test.js` and expect it to pass.',
      '',
      '## Task 2: Add direct tests for discovery handler shims',
      '',
      '- [ ] **Step 1: Read discovery shim module**',
      '',
      '    Read `server/handlers/discovery-handlers.js` to identify exported route helpers. Estimated scope: one source file and one new test file. Acceptance criteria: the shim delegates to the canonical discovery service.',
      '',
      '- [ ] **Step 2: Create test file for discovery handler shims**',
      '',
      '    Create `server/tests/discovery-template-handlers.test.js` with ~4 tests for discovery handler delegation and missing-container behavior. Validation: run `npx vitest run server/tests/discovery-template-handlers.test.js` and expect it to pass.',
    ].join('\n');
    ...
    expect(normalized).toContain('server/tests/budget-handlers.test.js');
    expect(normalized).toContain('server/tests/discovery-template-handlers.test.js');
    expect(normalized).not.toContain('agent/tests/run.test.js');
    const deterministic = planQualityGate.runDeterministicRules(normalized, { repoPath: projectDir });
    expect(deterministic.hardFails.find(f => f.rule === 'task_create_targets_unique')).toBeUndefined();
  });
  ...
  it('does not globally qualify dashboard plan verbs with the first referenced file', async () => {
    ...
    expect(normalized).toContain('Update dashboard/src/views/Approvals.jsx to integrate ErrorBoundary and LoadingSkeleton');
    expect(normalized).toContain('Enhance dashboard/src/components/ErrorBoundary.jsx to improve error display');
    expect(normalized).not.toContain('Update dashboard/src/views/Approvals.jsx to support dashboard views');
    expect(normalized).not.toContain('Integrate dashboard/src/views/Approvals.jsx into dashboard components');
    expect(normalized).not.toContain('Extend dashboard/src/views/Approvals.jsx to cover error and loading states');
