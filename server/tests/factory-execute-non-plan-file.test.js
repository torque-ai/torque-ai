...
`, 'utf8');
  ...
  it('does not route heavyweight validation through torque-remote when remote tests are disabled', async () => {
    ...
    expect(normalized).toContain('`dotnet test simtests/SimCore.DotNet.Tests.csproj -c Release --filter LanStartupCoordinatorTests`');
    ...
    expect(deterministic.hardFails).toEqual(expect.arrayContaining([
      expect.objectContaining({ rule: 'task_avoids_local_heavy_validation' }),
    ]));
  });
