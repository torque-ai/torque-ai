...
describe('v2-local-providers selection and payload behavior', () => {
  ...
  it('initializes hashline provider with configured defaults', () => {
    const { providers } = loadProviders({
      db: {
        getConfig: vi.fn((key) => {
          if (key === 'ollama_model') return 'db-default:14b';
          if (key === 'hashline_capable_models') return '';
          return null;
        }),
      },
    });
  ...
  it('falls back to the best hashline-capable model when the request is not allowed', async () => {
    const host = makeHost({ url: 'http://hashline-host:11434', models: ['qwen2.5-coder:14b'] });
    const { providers } = loadProviders({
      db: {
        getConfig: vi.fn((key) => {
          if (key === 'hashline_capable_models') return 'qwen2.5-coder';
          return null;
        }),
        listOllamaHosts: vi.fn(() => [host]),
        getAggregatedModels: vi.fn(() => ['mistral:7b', 'qwen2.5-coder:14b']),
        selectOllamaHostForModel: vi.fn((model) => (model === 'qwen2.5-coder:14b' ? { host } : null)),
      },
    });
    const provider = new providers.HashlineOllamaProvider();

    await expect(provider._selectExecutionTarget('mistral:7b')).resolves.toEqual(expect.objectContaining({
      hostUrl: 'http://hashline-host:11434',
      model: 'qwen2.5-coder:14b',
    }));
  });

  it('throws when no hashline-capable fallback model exists', async () => {
    const { providers } = loadProviders({
      db: {
        getConfig: vi.fn((key) => {
          if (key === 'hashline_capable_models') return 'qwen2.5-coder';
        ...
        listOllamaHosts: vi.fn(() => [makeHost({ models: ['mistral:7b'] })]),
        getAggregatedModels: vi.fn(() => ['mistral:7b']),
        selectOllamaHostForModel: vi.fn(() => null),
        selectHostWithModelVariant: vi.fn(() => null),
      },
    });
    ...
    await expect(provider._selectExecutionTarget('mistral:7b')).rejects.toThrow(
      "No hashline-capable model available for 'mistral:7b'",
    );
  });

  it('recovers to an allowed hashline model when variant selection returns a disallowed one', async () => {
    const wrongHost = makeHost({ id: 'host-wrong', name: 'Wrong Host', url: 'http://wrong-host:11434', models: ['mistral:7b'] });
    const allowedHost = makeHost({ id: 'host-good', name: 'Good Host', url: 'http://good-host:11434', models: ['qwen2.5-coder:7b'] });
    const { providers } = loadProviders({
      db: {
        getConfig: vi.fn((key) => {
          if (key === 'hashline_capable_models') return 'qwen2.5-coder';
        ...
        listOllamaHosts: vi.fn(() => [wrongHost, allowedHost]),
        getAggregatedModels: vi.fn(() => ['qwen2.5-coder:7b']),
        selectOllamaHostForModel: vi.fn((model) => {
          if (model === 'qwen2.5-coder:7b') return { host: allowedHost };
          if (model === 'mistral:7b') return { host: wrongHost };
          return null;
        }),
        selectHostWithModelVariant: vi.fn((model) => {
          if (model === 'qwen2.5-coder') return { host: wrongHost, model: 'mistral:7b' };
          return null;
        }),
      },
    });
    ...
    await expect(provider._selectExecutionTarget('qwen2.5-coder')).resolves.toEqual(expect.objectContaining({
      hostUrl: 'http://good-host:11434',
      model: 'qwen2.5-coder:7b',
    }));
  });

  it('filters listModels for hashline-capable models only', async () => {
    const { providers } = loadProviders({
      db: {
        getConfig: vi.fn((key) => {
          if (key === 'hashline_capable_models') return 'qwen2.5-coder';
        ...
        listOllamaHosts: vi.fn(() => [
          makeHost({ models: ['qwen2.5-coder:7b', 'mistral:7b', 'qwen2.5-coder:14b'] }),
        ]),
      },
    });
    const provider = new providers.HashlineOllamaProvider();

    await expect(provider.listModels()).resolves.toEqual(['qwen2.5-coder:14b', 'qwen2.5-coder:7b']);
  });
});
