...
describe('execute-hashline.js', () => {
  ...
  describe('executeHashlineOllamaTask', () => {
    ...
    it('calls workflow termination after successful hashline-openai completion', async () => {
      const srcDir = path.join(testDir, 'srcWfOpenAI');
      fs.mkdirSync(srcDir, { recursive: true });
      const targetRel = 'srcWfOpenAI/target.js';
      const targetPath = path.join(srcDir, 'target.js');
      const originalLine = 'const items = [];';
      fs.writeFileSync(targetPath, `${originalLine}\n`, 'utf8');
      const lineHash = computeLineHash(originalLine);

      const generated = [
        `HASHLINE_EDIT ${targetRel}`,
        `REPLACE L001:${lineHash} TO L001:${lineHash}`,
        originalLine,
        'END_REPLACE'
      ].join('\n');
      ...
      const fetchSpy = vi.spyOn(global, 'fetch').mockResolvedValue({
        ...
        json: vi.fn().mockResolvedValue({
          output: [{ type: 'message', content: [{ type: 'output_text', text: generated }] }],
      ...
      try {
        await mod.executeHashlineOpenaiTask({
          id: taskId,
          task_description: `Fix ${targetRel}`,
          model: 'gpt-4o-mini',
          working_directory: testDir,
    ...
    it('does not call workflow termination when hashline-openai falls back without edits', async () => {
      const srcDir = path.join(testDir, 'srcWfOpenAIFallback');
      fs.mkdirSync(srcDir, { recursive: true });
      fs.writeFileSync(path.join(srcDir, 'target.js'), 'export const value = 1;\n', 'utf8');
      const targetRel = 'srcWfOpenAIFallback/target.js';
      ...
      const fetchSpy = vi.spyOn(global, 'fetch').mockResolvedValue({
        ...
        status: 200,
        json: vi.fn().mockResolvedValue({
          output: [{ type: 'message', content: [{ type: 'output_text', text: 'no changes needed' }] }],
          usage: { input_tokens: 1, output_tokens: 2, total_tokens: 3 }
        }),
        text: vi.fn().mockResolvedValue(''),
      });
      ...
      expect(fallback).toHaveBeenCalledWith(
        taskId,
        expect.anything(),
        expect.stringContaining('no edits parsed from OpenAI response')
      );
      expect(handleWorkflowTermination).not.toHaveBeenCalled();
    });
