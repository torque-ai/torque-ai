...
describe('Host distribution under load', () => {
  ...
  it('VRAM-aware scheduling: large model routes to host with capacity', () => {
    registerMockHost(ctx.db, 'http://small.local:11434', ['codellama:latest', TEST_MODELS.QUALITY], { name: 'small-host', maxConcurrent: 2, id: 'small-host' });

    // Verify isLargeModelBlockedOnHost works
    // With no running tasks, 32b model should NOT be blocked
    const check1 = ctx.tm.isLargeModelBlockedOnHost(TEST_MODELS.QUALITY, 'small-host');
    ...
    const check2 = ctx.tm.isLargeModelBlockedOnHost(TEST_MODELS.QUALITY, 'small-host');
