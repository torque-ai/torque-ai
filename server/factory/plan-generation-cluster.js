...
function createPlanGenerationCluster(deps = {}) {
  ...
  function rewriteDisabledRemoteValidationLine(line) {
  ...
  function rewriteDisabledRemoteValidationCommands(value) {
    const lines = String(value || '').split(/\r?\n/);
    let changed = false;
    const rewritten = lines.map((line) => {
      const next = rewriteDisabledRemoteValidationLine(line);
  ...
  function routeHeavyValidationCommands(value, project = null) {
    if (isRemoteVerificationDisabled(project)) {
      return rewriteDisabledRemoteValidationCommands(value);
    }
    const lines = String(value || '').split(/\r?\n/);
    let changed = false;
  ...
  function normalizeAutoGeneratedPlanMarkdown(markdown, workItem, project) {
    ...
    taskSection = routeHeavyValidationCommands(taskSection, project);
    taskSection = dedupeValidationCommandTargets(routeHeavyValidationCommands(augmentAutoGeneratedTaskSpecificity(taskSection, workItem, project), project));
    const lines = [
