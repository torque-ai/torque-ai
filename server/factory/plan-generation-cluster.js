...
const { buildWorkItemSuccessDetail, buildWorkItemValidationDetail } = require('./plan-builders/workitem-spec');
...
const PLAN_GENERATION_IMPLEMENTATION_SIGNAL_RE = /\b(?:add|consume|cover|create|define|expose|export|extract|fix|guard|harden|implement|improve|introduce|parse|persist|refactor|repair|support|test|validate|wire)\b/i;
...
const PLAN_GENERATION_READ_ONLY_TASK_TITLE_RE = /^\s*(?:analy[sz]e|confirm|examine|identify|inspect|investigate|locate|read|research|review|run|search|verify)\b/i;
const PLAN_GENERATION_CHANGE_INTENT_RE = /\b(?:add|adjust|change|codify|cover|create|delete|document|edit|extend|fix|implement|introduce|modify|prune|record|refactor|remove|rename|repair|replace|rewrite|update|wire)\b/i;
...
function createPlanGenerationCluster(deps = {}) {
  ...
  function rewriteDisabledRemoteValidationLine(line, explicitWorkItemValidation = null) {
    const raw = String(line || '');
    if (!/\btorque-remote\b/i.test(raw) && !findHeavyLocalValidationCommand(raw)) {
      return raw;
    }
    if (lineContainsCommand(raw, explicitWorkItemValidation)) {
      return raw;
    }
    if (/`[^`]*(?:torque-remote|dotnet\s+(?:build|test)|scripts[\\/](?:build|test)\.(?:ps1|sh))[^`]*`/i.test(raw)) {
      return raw.replace(
        /`[^`]*(?:torque-remote|dotnet\s+(?:build|test)|scripts[\\/](?:build|test)\.(?:ps1|sh))[^`]*`/gi,
        '`git diff --check`',
      );
    }
    if (/\b(?:run|execute|validation|validate)\b/i.test(raw)) {
      return raw.replace(
        /\b(?:Run|Execute|Validate(?: with)?|Validation:)\b[^.。\n]*/i,
        (match) => `${match.split(/\s+/)[0]} \`git diff --check\``,
      );
    }
    return raw;
  }

  function rewriteDisabledRemoteValidationCommands(value, explicitWorkItemValidation = null) {
    const lines = String(value || '').split(/\r?\n/);
    let changed = false;
    const rewritten = lines.map((line) => {
      const next = rewriteDisabledRemoteValidationLine(line, explicitWorkItemValidation);
  ...
  function routeHeavyValidationCommands(value, project = null, workItem = null) {
    if (isRemoteVerificationDisabled(project)) {
      return rewriteDisabledRemoteValidationCommands(value, buildWorkItemValidationDetail(workItem));
    }
    const lines = String(value || '').split(/\r?\n/);
    let changed = false;
  ...
  function getLineSpecificPlanReferenceFile(line, workItem, planFilePaths = [], fallbackFile = null) {
    ...
    const basenameMatch = (planFilePaths || [])
      .slice()
  ...
  function qualifyAutoGeneratedPlanStructureLine(line, workItem, planFilePaths, fallbackFile) {
    ...
    out = replaceUnqualifiedPlanPhrase(out, /\bmodif(?:y|ies|ied|ying)\b/gi, `adjust behavior in ${fileRef}`);
  ...
  function qualifyAutoGeneratedPlanBodyLine(line, workItem, planFilePaths, fallbackFile) {
    if (isAutoGeneratedPlanStructureLine(line)) {
      return line;
    }
    if (!/\b(?:improv(?:e|es|ed|ing)|updat(?:e|es|ed|ing)|modif(?:y|ies|ied|ying)|clean\s+up|as\s+needed)\b/i.test(line)) {
      return line;
    }
  ...
  function qualifyAutoGeneratedVaguePlanLanguage(value, workItem) {
    ...
    text = text
      ...
      .map((part) => {
        ...
        const structured = qualifyAutoGeneratedPlanStructureLine(part, workItem, planFilePaths, primaryFile);
        return qualifyAutoGeneratedPlanBodyLine(structured, workItem, planFilePaths, primaryFile);
      })
      .join('');
    return text;
  }

  function isPureInvestigationTitle(title, workItem) {
    if (!PLAN_GENERATION_INVESTIGATION_TITLE_RE.test(title)) return false;
    const text = `${title}\n${workItem?.description || ''}`;
    if (/\b(?:and|then)\s+(?:add|cover|create|edit|fix|guard|harden|implement|improve|repair|test|validate|wire)\b/i.test(title)) {
      return false;
    }
    if (/\bAcceptance Criteria\b/i.test(text)) {
      return false;
    }
    return !/\b(?:fix|repair|implement|add|cover|guard|harden|wire)\b/i.test(text);
  }
  ...
  function workItemNeedsImplementationPlan(workItem) {
    ...
    return PLAN_GENERATION_IMPLEMENTATION_SIGNAL_RE.test(text)
      && !isPureInvestigationTitle(title, workItem);
  }
  ...
  function isGeneratedReadOnlyTask(task) {
    const title = String(task?.title || '').trim();
    const body = stripAutoGeneratedCommitSteps(task?.body || '');
    if (isGeneratedAnalysisOnlyTask({ ...task, body })) return true;
    if (!PLAN_GENERATION_READ_ONLY_TASK_TITLE_RE.test(title)) return false;
    return !PLAN_GENERATION_CHANGE_INTENT_RE.test(body);
  }

  function filterExistingProjectPaths(paths, project) {
    const isAllowedGeneratedPlanTarget = (file) => {
    ...
    if (!project?.path) {
      return [...new Set((paths || [])
        .map((file) => String(file || '').replace(/\\/g, '/'))
        .filter(isAllowedGeneratedPlanTarget))];
    }
    return [...new Set((paths || [])
      .map((file) => String(file || '').replace(/\\/g, '/'))
      .filter((file) => isAllowedGeneratedPlanTarget(file) && projectFileExists(project.path, file)))];
  }

  function buildTrackedFindingsTaskFromReadOnlyPlan(tasks, workItem, project, planFilePaths = []) {
    const evidencePath = `docs/superpowers/findings/work-item-${workItem?.id || 'current'}.md`;
    const referencedFiles = filterExistingProjectPaths(
      (planFilePaths || []).concat(
        (tasks || []).flatMap((task) => extractPlanDescriptionFilePaths(`${task?.title || ''}\n${task?.body || ''}`))
      ),
      project,
    ).slice(0, 5);
    const sourceList = referencedFiles.length > 0
      ? referencedFiles.map((file) => `\`${file}\``).join(', ')
      : 'the scoped repository evidence named by the work item';
    const successDetail = buildWorkItemSuccessDetail(workItem);
    const body = [
      '- [ ] **Step 1: Record scoped findings**',
      '',
      `    Create \`${evidencePath}\` with the current findings for work item #${workItem?.id || 'current'}, using ${sourceList}. Estimated scope: one tracked findings document plus up to ${Math.max(1, referencedFiles.length)} referenced file${referencedFiles.length === 1 ? '' : 's'}. Acceptance criteria: the document must state what was checked, the concrete gap or no-op conclusion, and the next implementation target if code changes remain. It must satisfy this work item outcome: ${successDetail}.`,
      '',
      '- [ ] **Step 2: Validate targeted change**',
      '',
      `    Run \`git diff --check\` from the repository root. Success criteria: formatting should pass and failures must be tied to \`${evidencePath}\`.`,
    ].join('\n');

    return {
      title: `Record findings for work item ${workItem?.id || 'current'}`,
      body,
    };
  }
  ...
  function buildAnalysisImplementationPathContext(task, workItem, project, planFilePaths = []) {
    ...
    const contextPaths = filterExistingProjectPaths(trustedContextPaths, project);
    ...
    return {
      paths: filterExistingProjectPaths(getTaskSpecificityFilePaths(task, workItem, planFilePaths), project),
  ...
  function buildImplementationTaskFromAnalysis(task, workItem, project, planFilePaths = []) {
    ...
    if (uniquePaths.length === 0) {
      return buildTrackedFindingsTaskFromReadOnlyPlan([task], workItem, project, planFilePaths);
    }
  ...
  function normalizeAutoGeneratedImplementationTaskShape(taskSection, workItem, project) {
    ...
    let changed = false;
    let synthesizedImplementationTask = null;
    ...
    for (const task of tasks) {
      ...
      if (needsImplementation && isGeneratedReadOnlyTask({ ...task, body: strippedBody })) {
        if (!synthesizedImplementationTask) {
          synthesizedImplementationTask = buildImplementationTaskFromAnalysis({ ...task, body: strippedBody }, workItem, project, planFilePaths);
          normalizedTasks.push(synthesizedImplementationTask);
        }
        changed = true;
        continue;
      }
    ...
    if (normalizedTasks.length === 0) {
      changed = true;
      normalizedTasks.push(buildTrackedFindingsTaskFromReadOnlyPlan(tasks, workItem, project, planFilePaths));
    }

    return changed ? rebuildAutoGeneratedTaskSection(normalizedTasks) : taskSection;
  }

  function pruneReadOnlyTitledGeneratedTasks(taskSection, workItem, project) {
    const tasks = parseAutoGeneratedTaskSectionBlocks(taskSection);
    if (tasks.length === 0) return taskSection;

    const filteredTasks = tasks.filter((task) => !PLAN_GENERATION_READ_ONLY_TASK_TITLE_RE.test(String(task?.title || '').trim()));
    if (filteredTasks.length === tasks.length) return taskSection;

    if (filteredTasks.length === 0) {
      const planFilePaths = extractPlanDescriptionFilePaths(taskSection);
      const replacement = workItemNeedsImplementationPlan(workItem)
        ? buildImplementationTaskFromAnalysis(tasks[0], workItem, project, planFilePaths)
        : buildTrackedFindingsTaskFromReadOnlyPlan(tasks, workItem, project, planFilePaths);
      return rebuildAutoGeneratedTaskSection([replacement]);
    }

    return rebuildAutoGeneratedTaskSection(filteredTasks);
  }
  ...
  function normalizeAutoGeneratedPlanMarkdown(markdown, workItem, project) {
    ...
    taskSection = pruneReadOnlyTitledGeneratedTasks(taskSection, workItem, project);
    ...
    taskSection = routeHeavyValidationCommands(taskSection, project, workItem);
    taskSection = dedupeValidationCommandTargets(routeHeavyValidationCommands(augmentAutoGeneratedTaskSpecificity(taskSection, workItem, project), project, workItem));
    const lines = [
