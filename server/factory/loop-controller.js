...
function buildAutoGeneratedPlanPrompt(project, workItem, priorFeedback = null) {
  ...
  // advert and the registered tools stay in lockstep.
  const codegraphEnabled = process.env.TORQUE_CODEGRAPH_ENABLED !== '0';
...
function normalizeAutoGeneratedPlanMarkdown(markdown, workItem, project) {
  ...
  const taskSection = routeHeavyValidationCommands(trimPromptEchoTail(raw.slice(taskMatch.index).trim()));
  const lines = [
