...
async function executeNonPlanFileStage(project, instance, workItem) {
  ...
  try {
    const submitResult = await handleSmartSubmitTask({
      ...
      // Plan generation is a pure text-generation task — the classifier
      // detects this from the prompt and maps it to the active routing
      // template's "plan_generation" rule. Do NOT hardcode a provider
      // here: doing so bypasses the template system, which is the factory's
      // accountable dispatch layer. If a specific project needs to override
      // the provider, edit its active template via /torque-templates.
      working_directory: project.path || process.cwd(),
      timeout_minutes: 10,
