...
const { createLoopStageDispatcher, createTransitionHelpers } = require('./transitions');
...
const { executeSenseStage, handleSenseTransition } = createSenseStage({
  ...stageSharedContext,
  LOOP_STATES,
...
const { executePlanStage, handlePlanTransition } = createPlanStage({
  ...stageSharedContext,
  LOOP_STATES,
  architectRunner,
  factoryIntake,
  fs,
  getHandleExecuteTransition: () => handleExecuteTransition,
...
const {
  ...
  fs,
  getClosedWorkItemLoopStopReason,
  getCurrentLoopState,
  getDecisionBatchId,
  getDecisionRowWorkItemId,
  getFactorySubmissionBatchId,
  getInstanceOrThrow,
  getMaybeShipNoop: () => maybeShipNoop,
  getMaybeShortCircuitZeroDiffExecute: () => maybeShortCircuitZeroDiffExecute,
  ...
  getProjectConfigForPlanGate,
  getProjectOrThrow,
  getRunExecuteVerifyStage: () => runExecuteVerifyStage,
  getSelectedWorkItem,
...
const dispatchAdvanceLoopStage = createLoopStageDispatcher({
  LOOP_STATES,
  handlers: {
...
async function runAdvanceLoop(instance_id) {
  ...
  const stageTransition = await dispatchAdvanceLoopStage({
    project,
    instance,
    currentState,
    previousState,
  ...
  if (stageTransition?.finalResult) {
    return stageTransition.finalResult;
  }
  instance = stageTransition?.instance || instance;
  stageResult = stageTransition?.stageResult ?? null;
  transitionReason = stageTransition?.transitionReason ?? null;
  if (Object.prototype.hasOwnProperty.call(stageTransition || {}, 'transitionWorkItem')) {
    transitionWorkItem = stageTransition.transitionWorkItem;
  }
