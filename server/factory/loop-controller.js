...
function buildAutoGeneratedPlanPrompt(project, workItem, priorFeedback = null) {
  ...
  const prompt = [
    ...
    '- Do not create analysis-only tasks. Avoid task titles whose main action is identify, analyze, locate, inspect, examine, research, investigate, or review. Put necessary reading inside the implementation step for the file being changed.',
...
function buildImplementationTaskFromAnalysis(task, workItem, project, planFilePaths = []) {
  ...
  const scopedFiles = (implementationFiles.length > 0 ? implementationFiles : uniquePaths).slice(0, 4);
