...
// Codegraph integration block. Emitted when the plugin is enabled
// (default-on; TORQUE_CODEGRAPH_ENABLED=0 opts out) — otherwise the
...
function buildAutoGeneratedPlanPrompt(project, workItem, priorFeedback = null) {
  ...
  // Default-on. Tracks the plugin's own isFeatureEnabled() so the prompt
  // advert and the registered tools stay in lockstep. Phase G: also
  // disabled for the ollama lane regardless of env var — small local
  // models can't drive the cg_* tools effectively.
  const codegraphEnabled = process.env.TORQUE_CODEGRAPH_ENABLED !== '0' && !useOllamaShortPrompt;
