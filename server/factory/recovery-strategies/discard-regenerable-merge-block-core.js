...
// Shared logic for the merge-target-dirty discard recovery, used by both:
//   - B1 strategy (server/factory/recovery-strategies/discard-regenerable-merge-block.js)
//     dispatched via reject_reason pattern match in replan-recovery sweep
//   - A strategy (server/plugins/auto-recovery-core/strategies/discard-regenerable-merge-block.js)
//     dispatched via auto-recovery engine when learn_merge_target_dirty rule classifies
//     a project pause
//
// Defense-in-depth handler: when a merge of worktree → master fails because
// master has uncommitted/untracked files, classify the dirty paths against
// an allowlist of "files the factory might regenerate during a cycle and we
// can safely discard before retrying the merge". If everything dirty is
// allowlisted, run `git checkout -- <files>` (tracked) or fs.rmSync (untracked)
// and signal retry. If anything outside the allowlist is dirty, refuse —
// real operator work in progress.
//
// What this deliberately does NOT do: discard any tracked source file.
// The operator-blocked-merge semantics still hold for real uncommitted
// work — that requires human triage.
...
// High-level: classify the merge target's dirty state, discard if every
// dirty entry is on the regenerable allowlist, refuse otherwise. Returns
// a normalized result that both A and B1 wrappers translate into their
// respective contracts.
//
// Outcomes:
//   'clean'         — merge target was already clean when we looked
//   'discarded'     — dirty entries existed and were all allowlisted; we removed them
//   'refused'       — non-regenerable files dirty (real operator work) OR
//                     allowlisted paths in unhandled status (staged/unmerged) OR
//                     discard operations failed mid-flight
//   'unavailable'   — repo path missing or git status failed
async function attemptDiscard(repoRoot, { logger, contextLog = {} } = {}) {
  if (!repoRoot) {
    return {
      outcome: 'unavailable',
      reason: 'merge_target_dirty_discard: repo path unavailable',
    };
  }
  ...
  if (result.failures.length > 0) {
    ...
    return {
      ...
      details: { failures: result.failures, restored: result.restored, removed: result.removed },
    };
  }
  ...
  return {
    ...
    details: { restored: result.restored, removed: result.removed },
  };
}

module.exports = {
  REGENERABLE_PATH_PATTERNS,
  DISCARDABLE_STATUS_CODES,
  parsePorcelainLine,
  spawnGit,
  listDirtyFiles,
