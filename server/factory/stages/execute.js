...
function createExecuteStage({
  ...
  getInstanceOrThrow,
  getMaybeShipNoop,
  getMaybeShortCircuitZeroDiffExecute,
  ...
  assertFunction(getClosedWorkItemLoopStopReason, 'getClosedWorkItemLoopStopReason');
  ...
  assertFunction(getMaybeShipNoop, 'getMaybeShipNoop');
  assertFunction(getMaybeShortCircuitZeroDiffExecute, 'getMaybeShortCircuitZeroDiffExecute');
  assertFunction(getProjectConfigForPlanGate, 'getProjectConfigForPlanGate');
  assertFunction(getRunExecuteVerifyStage, 'getRunExecuteVerifyStage');
  ...
  assertFunction(restoreTerminalEscalationWorkItem, 'restoreTerminalEscalationWorkItem');
  ...
  async function handleExecuteTransition({
    ...
    const maybeShortCircuitZeroDiffExecute = getMaybeShortCircuitZeroDiffExecute();
    const maybeShipNoop = getMaybeShipNoop();
    const runExecuteVerifyStage = getRunExecuteVerifyStage();
    assertFunction(maybeShortCircuitZeroDiffExecute, 'getMaybeShortCircuitZeroDiffExecute()');
    assertFunction(maybeShipNoop, 'getMaybeShipNoop()');
    assertFunction(runExecuteVerifyStage, 'getRunExecuteVerifyStage()');
      ...
      if (preExecuteZeroDiff.advance_to_verify) {
        return {
          finalResult: buildLoopAdvanceResult({
            ...
            instance_id,
            previousState,
            newState: LOOP_STATES.VERIFY,
            pausedAtStage: null,
            stageResult: preExecuteZeroDiff.stage_result,
        ...
        if (generated.next_state === LOOP_STATES.IDLE) {
          ...
          return {
            finalResult: buildLoopAdvanceResult({
              ...
              newState: LOOP_STATES.IDLE,
              pausedAtStage: null,
              stageResult: generated.stage_result || null,
              reason: generated.reason || 'stop_execution_idle',
            }),
          };
        }
      ...
      if (executeStage.next_state === LOOP_STATES.IDLE) {
        ...
        return {
          finalResult: buildLoopAdvanceResult({
            ...
            newState: LOOP_STATES.IDLE,
            pausedAtStage: null,
            stageResult: executeStage.stage_result || null,
            reason: transitionReason || executeStage.reason || 'execute_stop_idle',
          }),
        };
      }
        ...
        if (zeroDiff.advance_to_verify) {
          return {
            finalResult: buildLoopAdvanceResult({
              ...
              newState: LOOP_STATES.VERIFY,
              pausedAtStage: null,
              stageResult: zeroDiff.stage_result,
