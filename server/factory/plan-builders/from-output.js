...
function buildPlanFromFileEditsProposal(rawOutput, workItem, project) {
  ...
  const validationCommand = buildTaskValidationDetail(
    {
      title: `Apply proposed edits for ${workItem?.title || `work item ${workItem?.id}`}`,
      body: edits.map((edit) => `Edit \`${edit.file}\``).join('\n'),
    },
    workItem,
    project,
    edits.map((edit) => edit.file),
  ) || inferAutoGeneratedPlanValidationCommand(project?.path);
