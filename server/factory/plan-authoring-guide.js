...
function composeGuide({ rulesSource = RULES, examplesBlock = DEFAULT_EXAMPLES } = {}) {
  const lines = [
    ...
    'the first pass.',
    'Leave full-project verify/build commands to the orchestrator. If a task',
    'truly needs heavyweight validation in its body, follow the project',
    'verification policy; do not add `torque-remote` unless remote validation',
    'is explicitly enabled for that project.',
    '',
