...
const { buildAutoGeneratedPlanPath } = require('./shared/plan-path');
...
function createPlanExecuteStage(deps = {}) {
  ...
  async function executePlanFileStage(project, instance, workItem) {
    ...
    const generatedPlanSnapshot = (() => {
      const planPath = targetItem.origin?.plan_path || null;
      if (!planPath || !shouldDeletePlanPathForNeedsReplan(targetItem, planPath)) {
        return null;
      }
      try {
        return {
        ...
        logger.warn('EXECUTE stage: could not snapshot generated plan before worktree preparation', {
          project_id: project.id,
          work_item_id: targetItem.id,
          plan_path: planPath,
          err: err.message,
    ...
    if (generatedPlanSnapshot && executionWorkingDirectory) {
      const currentPlanPath = targetItem.origin?.plan_path || generatedPlanSnapshot.sourcePath;
      const restorePlanPath = executionWorkingDirectory !== project.path
        && !isPathInside(executionWorkingDirectory, currentPlanPath)
        ? buildAutoGeneratedPlanPath(project, targetItem, executionWorkingDirectory)
        : currentPlanPath;
      if (restorePlanPath && !fs.existsSync(restorePlanPath)) {
        try {
          fs.mkdirSync(path.dirname(restorePlanPath), { recursive: true });
          fs.writeFileSync(restorePlanPath, generatedPlanSnapshot.markdown);
          if (path.resolve(restorePlanPath) !== path.resolve(currentPlanPath)) {
            const nextOrigin = {
              ...getWorkItemOriginObject(targetItem),
              plan_path: restorePlanPath,
            };
            targetItem = factoryIntake.updateWorkItem(targetItem.id, {
              origin_json: nextOrigin,
            }) || {
          ...
          logger.info('EXECUTE stage: restored generated plan after worktree preparation', {
            project_id: project.id,
            work_item_id: targetItem.id,
            source_plan_path: generatedPlanSnapshot.sourcePath,
            restored_plan_path: restorePlanPath,
            worktree_path: executionWorkingDirectory,
          });
          safeLogDecision({
            project_id: project.id,
            stage: LOOP_STATES.EXECUTE,
            action: 'generated_plan_restored_after_worktree_prepare',
            reasoning: 'Restored a generated plan captured before worktree reclaim so EXECUTE can continue instead of routing the item back to needs_replan.',
            inputs: {
              ...getWorkItemDecisionContext(targetItem),
              source_plan_path: generatedPlanSnapshot.sourcePath,
            },
          ...
          logger.warn('EXECUTE stage: failed to restore generated plan after worktree preparation', {
            project_id: project.id,
            work_item_id: targetItem.id,
            source_plan_path: generatedPlanSnapshot.sourcePath,
            restored_plan_path: restorePlanPath,
