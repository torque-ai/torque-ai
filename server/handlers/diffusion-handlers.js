...
function buildStarvationRecoveryScoutPrompt({
  ...
  const user = [
    ...
    '- If the exact `list_directory` / `search_files` tools are unavailable and you must use shell commands, use simple single commands only.',
    '- Do not use PowerShell pipelines, `Select-Object`, redirection, command substitution, or chained commands.',
    '- Prefer commands shaped like `rg --files`, `rg -n TODO`, `Get-ChildItem -Name`, or `Get-Content path -TotalCount 120`.',
    '',
    '## Required Output',
