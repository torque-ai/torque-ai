...
/**
 * Lazy-load the agent registry to avoid circular dependency with index.js.
 * index.js requires tools.js -> handler modules at startup, so a top-level
 * require('../index') would trigger main() during module load.
 *
 * Exported as _getRegistry so tests can override without vi.mock.
 */
...
function _buildExecutionResponse(result, summary) {
  const lines = [summary];

  lines.push(`Execution: ${result.remote ? 'remote' : 'local'}`);
  ...
  if (result.output) {
    lines.push('', 'STDOUT:', result.output);
  }
...
function _toExecutionToolResult(result, summary) {
  if (_isCoreError(result)) {
    return makeError(result.error_code, result.error, result.details || null);
  }

  return _buildExecutionResponse(result, summary);
}

// ============================================
// REMOTE AGENT MANAGEMENT HANDLERS
// ============================================

/**
 * Register a remote execution agent.
...
function handleRegisterRemoteAgent(args) {
  ...
  const registry = module.exports._getRegistry();
  if (!registry) {
    return makeError(ErrorCodes.OPERATION_FAILED, 'Agent registry not initialized');
  }

  const existingAgent = typeof registry.get === 'function' ? registry.get(id) : null;
  const { tls, rejectUnauthorized } = _resolveTlsSettings(args, existingAgent);

  registry.register({ id, name, host, port, secret, max_concurrent, tls, rejectUnauthorized });
...
/**
 * Remove a registered remote agent by ID.
 * @param {Object} args - Handler arguments.
 * @returns {Object} Response payload.
 */
function handleRemoveRemoteAgent(args) {
  ...
  const existing = registry.get(agent_id);
  if (!existing) {
...
/**
 * Check health of one or all remote agents.
 * @param {Object} args - Handler arguments.
...
async function handleCheckRemoteAgentHealth(args) {
  try {
    ...
    if (agent_id) {
      const client = registry.getClient(agent_id);
      if (!client) {
    ...
    const lines = results.map(r =>
      `${r.id}: ${r.status}${r.failures ? ` (${r.failures} failures)` : ''}`
    );
    return { content: [{ type: 'text', text: lines.join('\n') }] };
  } catch (err) {
...
/**
 * Ask a specific remote agent to claim a task.
 * @param {Object} args - Handler arguments.
 * @returns {Promise<Object>} Response payload.
 */
async function handleClaimTaskOnAgent(args = {}) {
  try {
    const { agent_id, task_id, lease_seconds } = args;

    if (!agent_id || !task_id) {
      return makeError(ErrorCodes.MISSING_REQUIRED_PARAM, 'Required: agent_id, task_id');
    }
    ...
    if (typeof client.claimTask !== 'function') {
      return makeError(ErrorCodes.OPERATION_FAILED, 'Agent client does not support task claiming');
    }

    const result = await client.claimTask(task_id, lease_seconds);
    if (!result || result.success === false) {
      return {
        content: [{
          type: 'text',
          text: `Failed to claim task ${task_id} on agent ${agent_id}`
        }],
        isError: true,
      };
    }
    ...
    return {
      content: [{
        type: 'text',
        text: `Claimed task ${task_id} on agent ${agent_id}`
      }]
    };
  } catch (err) {
    return makeError(ErrorCodes.INTERNAL_ERROR, err.message || String(err));
...
/**
 * Record a heartbeat payload reported by an agent.
 * @param {Object} args - Handler arguments.
 * @returns {Promise<Object>} Response payload.
 */
async function handleRecordAgentHeartbeat(args = {}) {
  try {
    const { agent_id, heartbeat } = args;
    ...
    if (typeof client.recordHeartbeat !== 'function') {
      return makeError(ErrorCodes.OPERATION_FAILED, 'Agent client does not support heartbeat recording');
    }

    await client.recordHeartbeat(heartbeat || {});
    return {
      content: [{
        type: 'text',
        text: `Recorded heartbeat for ${agent.name}`
      }]
    };
  } catch (err) {
    return makeError(ErrorCodes.INTERNAL_ERROR, err.message || String(err));
...
/**
 * Run an arbitrary command through the remote test router with local fallback.
 * Returns a plain object for reuse by REST handlers.
 * @param {Object} args - Handler arguments.
 * @returns {Promise<Object>} Core execution result.
 */
...
/**
 * Run a shell command on a healthy remote agent, falling back to local execution.
 * @param {Object} args - Handler arguments.
 * @returns {Promise<Object>} Response payload.
 */
async function handleRunRemoteCommand(args = {}) {
  try {
    if (_isHttpInvocation(args, arguments[1])) {
      return _handleRemoteExecutionHttpRequest(args, arguments[1], module.exports.runRemoteCommandCore);
    }
    ...
    const registry = module.exports._getRegistry();
    const healthyAgent = _findHealthyAgent(registry);
    ...
    const client = typeof registry?.getClient === 'function'
      ? registry.getClient(healthyAgent.id)
      : null;
...
/**
 * Run the configured project verify command through the remote test router.
 * Returns a plain object for reuse by REST handlers.
 * @param {Object} args - Handler arguments.
 * @returns {Promise<Object>} Core execution result.
 */
...
/**
 * Run the configured project verify command through the MCP remote-command path.
 * @param {Object} args - Handler arguments.
 * @returns {Promise<Object>} Response payload.
 */
async function handleRunTests(args = {}) {
  try {
    if (_isHttpInvocation(args, arguments[1])) {
      return _handleRemoteExecutionHttpRequest(args, arguments[1], module.exports.runTestsCore);
    }
    ...
    return module.exports.handleRunRemoteCommand({
...
const registerRemoteAgent = handleRegisterRemoteAgent;
const listRemoteAgents = handleListRemoteAgents;
...
module.exports = {
  ...
  handleRunRemoteCommand,
  handleRunTests,
  // Aliases for backward compatibility (used by tests and external consumers)
  registerRemoteAgent,
  listRemoteAgents,
  getRemoteAgent: handleGetRemoteAgent,
  deleteRemoteAgent,
