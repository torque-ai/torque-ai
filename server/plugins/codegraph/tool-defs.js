...
const tools = [
  ...
  {
    name: 'cg_diff',
    description: 'Compute the symbol-level delta between two git commits in a repo. Re-extracts symbols from each changed file at both shas (added files: extract at to_sha; deleted: extract at from_sha; modified or renamed-with-both-sides-indexable: extract both and set-diff). Returns `{from_sha, to_sha, added_symbols: [{name, kind, file, line, container?}], removed_symbols: [...], signature_changed_symbols: [{name, kind, file, line, container?, from_flags, to_flags, changed}], changed_files: {added, modified, deleted, renamed: [{from, to}]}, skipped_files, parse_errors, truncated, max_files, total_files_changed}`. Symbol identity is (name, kind, container, stable-file): line numbers do NOT affect identity, and a content-stable file rename uses the new path on both sides so it surfaces as 0 added / 0 removed (the rename itself appears in changed_files.renamed). Boolean signature flags (is_async, is_exported, is_static, is_generator) are NOT part of identity — flips appear in signature_changed_symbols so a sync→async breaking change is visible without conflating it with a true add+remove. parse_errors lists files in the diff scope whose extractor threw (mirrors the indexer\'s skip-and-continue policy). Bounded to changed files only (fast for typical commits) and capped at max_files (default 500); over the cap returns truncated:true with no symbol diff. Reads from git object store directly — does NOT depend on cg_index_state.',
    inputSchema: {
      type: 'object',
      properties: {
        repo_path: { type: 'string' },
