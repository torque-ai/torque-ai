...
| `mcp-launch-readiness.js` | Starts the MCP gateway, verifies it comes up cleanly, then runs dual-agent smoke and a launch-scoped readiness pack. | Run `npm run ci:mcp-launch-readiness` from `server/`. |
| `mcp-readiness-pack.js` | Aggregates gateway health and MCP control artifacts into a single readiness report. Full control evidence is required by default; set `TORQUE_MCP_READINESS_REQUIRED` for narrower launch checks. | Run `npm run ci:mcp-readiness-pack` from `server/`. |
