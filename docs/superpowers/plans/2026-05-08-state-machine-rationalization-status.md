...
### 6. Loop instance restart recovery (sub-project 7 of 7)

Cutover commit on main: see `git log --grep='retry ready-for loops on startup'`.

- `server/factory/startup-reconciler.js` now schedules a normal `advanceLoopAsync(..., { autoAdvance: true })` for active instances parked at `READY_FOR_<stage>` after restart.
- The loop keeps its paused target instead of restarting from SENSE.
- The retry remains bounded by normal stage occupancy and the `READY_FOR_<stage>` watchdog shipped in sub-project 2.
- Regression coverage proves startup reconciliation retries a `READY_FOR_PLAN` parked instance instead of skipping it.
