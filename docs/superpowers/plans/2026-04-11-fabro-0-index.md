...
31 plans implementing recommendations from `docs/superpowers/research/2026-04-11-fabro-review.md` plus session-derived enhancements plus features scouted from 14 other software-factory / AI-coding / orchestration projects (OpenHands, Aider, SWE-agent, MetaGPT, ChatDev, Plandex, Cline, Goose, GPT-Engineer, TaskWeaver, Sweep, DSPy, CrewAI, LangGraph, Temporal).
...
| 22 (sub-workflows) | 1 |
| 26 (crew/flow) | 5 (parallel), 18 (architect/editor), 23 (typed signatures) |
| 27 (typed state) | 14 (event backbone), 23 (typed signatures) |
| 28 (time-travel) | 10 (resume/replay), 27 (typed state), 29 (journal) |
...
**Batch F — Reusability & automation**
...
| 4 | Fabro |
| 5 | Fabro |
| 6 | session-derived (factory needs cost guardrails) |
| 7 | session-derived (per-task verify cuts noise) |
| 8 | session-derived (template reuse) |
| 9 | session-derived (overnight factory runs) |
| 10 | session-derived (survive restarts) |
| 11 | session-derived (deflake verify signal) |
| 12 | Fabro (mentioned but not deeply planned) |
| 13 | session-derived (visual graph rendering) |
| 14 | OpenHands |
