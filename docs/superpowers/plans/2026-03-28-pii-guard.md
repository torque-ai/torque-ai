...
describe('pii-guard', () => {
  ...
  describe('scanAndReplace', () => {
    ...
    it('replaces 172.31.x.x (upper bound)', () => {
      const result = piiGuard.scanAndReplace('Server: 172.31.5.200');
      expect(result.clean).toBe(false);
...
describe('pii_guard project config', () => {
  ...
  it('stores and retrieves pii_guard config via project_metadata', () => {
    ...
    expect(parsed.custom_patterns[0].pattern).toBe('Werem');
  });
...
describe('PII guard in output safeguards', () => {
  ...
  it('handles output with multiple PII types', () => {
    const output = [
      'Working in C:\\Users\\Werem\\Projects\\app',
      'Host: 192.168.1.100',
...
|----------|--------------|-------------|
| User paths | `C:\Users\Werem\Projects\...` | `C:\Users\<user>\Projects\...` |
| Private IPs | `192.168.1.100` | `192.0.2.100` |
| Emails | `user@gmail.com` | `user@example.com` |
...
Run: `cd server && node -e "const p = require('./utils/pii-guard'); console.log(p.scanAndReplace('C:\\\\Users\\\\Werem at 192.168.1.100'))"`
