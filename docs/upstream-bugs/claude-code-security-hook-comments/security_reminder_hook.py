...
Comment-stripping added to avoid false positives on code examples inside comments / docs.
"""
...
def _replace_with_spaces(match):
    """Replace matched comment text while preserving line boundaries."""
    text = match.group(0)
    return re.sub(r"[^\r\n]", " ", text)


def strip_comments(content, file_path):
    """Strip comments for supported file types before security substring checks."""
    ...
    # Replace comments with same-length spacing so warning line numbers stay stable.
    if extension in slash_comment_extensions:
        content = re.sub(r"/\*.*?\*/", _replace_with_spaces, content, flags=re.DOTALL)
        return re.sub(r"//[^\r\n]*", _replace_with_spaces, content)
    ...
    if extension in hash_comment_extensions:
        ...
        for index, line in enumerate(lines):
            if index == 0 and line.startswith("#!"):
                ...
                continue
            stripped_lines.append(re.sub(r"#[^\r\n]*", _replace_with_spaces, line))
        return "".join(stripped_lines)

    if extension in html_comment_extensions:
        return re.sub(r"<!--.*?-->", _replace_with_spaces, content, flags=re.DOTALL)
    ...
    for pattern in SECURITY_PATTERNS:
        ...
        if "substrings" in pattern and content:
            if stripped_content is None:
                stripped_content = strip_comments(content, file_path)
            for substring in pattern["substrings"]:
                if substring in stripped_content:
                    return pattern["ruleName"], pattern["reminder"]
