...
## Superseded by Current Audit (2026-05-09)

This document is preserved as historical analysis. The current source of truth is `docs/rest-parity-gap-report.md`, generated by:

```bash
node scripts/rest-parity-audit.js
```

Current result:

- MCP tools defined: **751**
- REST-applicable MCP tools: **750**
- MCP tools with REST mappings: **750**
- Actionable MCP-to-REST coverage gaps: **0**
- Intentional MCP-only tools: **1** (`ack_notification`, because it requires live MCP/SSE session state)
- REST routes pointing at non-existent tools: **0**

The 55-gap list below is stale and should not be used for current work.
