...
The rollup also distinguishes `ready` from `hands_off_ready`. `ready` means the project-level control plane can cycle autonomously. `hands_off_ready` additionally requires global project work to be enabled, no operator-owned queues to remain, and the factory tick scheduler to be armed for every ready project. Operator-owned queues include pending approval tasks, `needs_review` work items, and `escalation_exhausted` work items. For project- or status-scoped plans, pending approval task counts use factory project identifiers and target-project tags so unrelated approvals do not block the requested scope. The `manual_intervention` object reports those counts, unarmed tick project ids, reason codes, and capped per-project `work_item_blockers` arrays for `needs_review` / `escalation_exhausted` without mutating projects. Each blocker may include `oldest_created_at`, `oldest_updated_at`, and `newest_updated_at` timestamps plus capped `reject_reason_counts` so operators can prioritize stale review/escalation queues and see dominant causes without querying or reclassifying the work items.
