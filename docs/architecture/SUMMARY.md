...
Coverage: 1450/1526 tracked files indexed (95%), 0 pending.
...
- **Strengths:** Coverage is effectively complete at 95% with no pending files. The pack captures 5 canonical flows, which gives downstream models a usable execution map.

## Benchmark
- **Benchmark Readiness:** expert_ready (A, score 97).
- **Cases:** 9 pass, 1 partial, 0 fail.
...
- **Read Order:** `server/index.js`, `bin/torque.js`, `server/task-manager.js`, `server/tools.js`, `server/api/v2-dispatch.js`, and `server/handlers/task/index.js`.

## Suggested Entry Points
- `server/index.js` — Package main entrypoint.
- `bin/torque.js` — CLI entrypoint.
- `server/task-manager.js` — Top-level task execution coordinator.
- `server/tools.js` — Tool catalog and dispatch surface.
- `server/api/v2-dispatch.js` — Primary v2 control-plane dispatch surface.
- `server/execution/workflow-runtime.js` — Workflow runtime and unblocking.
...
- **Scheduled automation:** Schedules are stored in DB helpers, fired by the maintenance scheduler, and translated into tasks or tool executions by the schedule runner. Start with `server/db/cron-scheduling.js`, `server/api/v2-governance-handlers.js`, `server/maintenance/scheduler.js`, and `server/tools.js`.
...
## Major Subsystems
- **Control-plane API:** HTTP and transport surfaces that expose TORQUE task, provider, workflow, and governance operations. Coverage: 25/25 indexed. Representative files: `server/index.js`, `server/api-server.core.js`, and `server/api/v2-governance-handlers.js`. Key exports: `init` (8), `_testing` (2), and `buildV2Middleware` (2). Key dependencies: `server/api/middleware.js` (16) and `server/logger.js` (12).
- **Task execution pipeline:** Task startup, provider routing, process lifecycle, retries, verification, and completion handling. Coverage: 31/31 indexed. Representative files: `server/task-manager.js`, `server/execution/queue-scheduler.js`, and `server/execution/task-finalizer.js`. Key exports: `init` (14), `_testing` (2), and `getEffectiveGlobalMaxConcurrent` (2). Key dependencies: `server/logger.js` (20) and `child_process` (8).
...
- **Scheduled automation:** Schedules should create tracked task or tool executions; background automation should not fire invisibly. Read `server/db/cron-scheduling.js`, `server/api/v2-governance-handlers.js`, `server/maintenance/scheduler.js`, and `server/tools.js`.
...
- **Editing Provider adapters:** Provider registry, CLI/API adapters, prompts, and provider-specific execution logic. Start with `server/providers/execution.js`, `server/providers/adapter-registry.js`, and `server/providers/base.js`.

## Change Impact Guidance
- **Control-plane API:** Recheck `server/index.js`, `server/api-server.core.js`, `server/api/v2-governance-handlers.js`, and `server/api/middleware.js`. Validate with `npx vitest run server/tests/api-server-core.test.js server/tests/v2-governance-handlers.test.js server/tests/claude-event-hooks.test.js server/tests/api-middleware.test.js` and `npm run lint`.
...
- `server/logger.js` — High fan-in (179) and fan-out (3) inside Runtime core.
- `server/db/task-core.js` — High fan-in (152) and fan-out (11) inside Persistence and scheduling.
- `server/container.js` — High fan-in (35) and fan-out (160) inside Runtime core.
- `server/tests/test-helpers.js` — Shared internal dependency reused by 104 indexed modules in Validation and tests.
- `server/task-manager.js` — High fan-in (72) and fan-out (65) inside Task execution pipeline.

## Representative Tests
- **Task lifecycle:** `server/tests/host-distribution.test.js`, `server/tests/starttask-helpers.test.js`, `server/tests/task-pipeline-handlers.test.js`, and `server/tests/completion-pipeline.test.js`. Tests touch files that participate in this canonical flow.
...
- How do schedules and Run Now executions work? Read `server/db/cron-scheduling.js`, `server/api/v2-governance-handlers.js`, `server/maintenance/scheduler.js`, and `server/tools.js`. Schedules are stored in DB helpers, fired by the maintenance scheduler, and translated into tasks or tool executions by the schedule runner.
